...
type MenuCaterogiesWithProducts = Prisma.MenuCategoryGetPayload<{
  include: { products: true };
}>;

const RestaurantCategories = ({ restaurant }: RestaurantCategoriesProps) => {
  const [selectCategory, setSelectCategory] =
    useState<MenuCaterogiesWithProducts>(restaurant.menuCategories[0]);

  const { products, total, toggleCart, totalQuantity } =
    useContext(CartContext);

  const handleCategoryClick = (category: MenuCaterogiesWithProducts) => {
    setSelectCategory(category);
  };

  const getCategoryButtonVariant = (category: MenuCaterogiesWithProducts) => {
  ...
  return (
    <div className="relative z-50 mt-[-1.5rem] rounded-t-3xl bg-white">
      ...
      <h3 className="px-5 font-semibold">{selectCategory.name}</h3>
